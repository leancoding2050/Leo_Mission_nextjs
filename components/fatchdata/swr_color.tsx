...
const fetcher = (url: string): Promise<Color[]> =>
  fetch(url).then((res) => res.json());

export const SWR_Color = ({ field, disabled }: SWR_ColorProps) => {
  const { data, error, isLoading } = useSWR<Color[]>(
    "http://127.0.0.1:8000/api/mission/colors/",
    fetcher,
