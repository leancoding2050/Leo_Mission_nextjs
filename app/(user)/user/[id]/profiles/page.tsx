...
const ProfilesPage = () => {
  ...
  useEffect(() => {
    const getUserListsDatabyId = async (id: string) => {
      try {
        const res = await fetch(`/api/User_Lists_by_ID/${id}`);
        if (!res.ok) {
          throw new Error(`無法獲取用戶數據: ${res.status}`);
        }
        const result = await res.json();
        console.log("API 回傳數據:", result);
        const userData = Array.isArray(result) ? result : [result];
        setGetUserListsDatabyId(userData);
        setError(null);
      } catch (error) {
  ...
  if (error) {
    return <div className="text-center text-red-600 text-lg">錯誤: {error}</div>;
  }
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        {GetUserListsDatabyId.length > 0 ? (
          GetUserListsDatabyId.map((d) => (
            ...
            <div className="mt-4">
              {d.image && Array.isArray(d.image) && d.image.length > 0 && d.image[0]?.path ? (
                <Image
                  src={d.image[0].path}
                  alt={`頭像 for ${d.username}`}
                  width={192}
                  height={192}
                  className="object-cover rounded-md"
                />
              ) : (
                <p className="text-gray-700">無頭像</p>
              )}
            </div>
              <div className="mt-4 flex gap-4">
                <Link href={`/user/${d.id}/profiles/jobprogressLists`}>
          ...
          <div className="text-center text-gray-700">無用戶數據</div>
        )}
      </div>
    </div>
  );
