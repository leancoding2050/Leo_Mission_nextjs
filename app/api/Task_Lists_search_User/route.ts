...
export async function GET(req: Request) {
  try {
    ...
    let whereClauses: Prisma.TaskWhereInput = {
      AND: [
        { task_public: true }, // 只返回公開的 Task
        { OR: [] },
      ],
    };
    ...
    if (searchField === "all" || stringFields.includes(searchField)) {
      stringFields.forEach((field) => {
        if (searchField === "all" || searchField === field) {
          (whereClauses.AND as Prisma.TaskWhereInput[])[1].OR!.push({
            [field]: {
              contains: query,
    ...
    // 處理布林值字段（不包括 task_public）
    if (searchField === "all" || booleanFields.includes(searchField)) {
      booleanFields.forEach((field) => {
        if (searchField === "all" || searchField === field) {
          if (query.toLowerCase() === "true") {
            (whereClauses.AND as Prisma.TaskWhereInput[])[1].OR!.push({ [field]: true });
          } else if (query.toLowerCase() === "false") {
            (whereClauses.AND as Prisma.TaskWhereInput[])[1].OR!.push({ [field]: false });
          }
        }
      });
    }
    ...
    if (searchField === "all" || searchField === "task_price") {
      const numberQuery = parseInt(query, 10);
      if (!isNaN(numberQuery)) {
        (whereClauses.AND as Prisma.TaskWhereInput[])[1].OR!.push({ task_price: numberQuery });
      }
    }
    ...
    // 如果 OR 條件為空，只返回 task_public 為 true 的記錄
    if ((whereClauses.AND as Prisma.TaskWhereInput[])[1].OR!.length === 0) {
      whereClauses = { task_public: true };
    }
    ...
    // 如果沒有數據，返回訊息
    if (tasks.length === 0) {
      return NextResponse.json({ message: "沒有數據" }, { status: 200 });
    }
