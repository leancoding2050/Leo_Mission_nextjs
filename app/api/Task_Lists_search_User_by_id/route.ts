...
//       AND: [
//         { teacher: userNickname }, // 限制 teacher 必須與用戶的 nickname 匹配
//         {
//           OR: [],
//         },
//       ],
...
export async function GET(req: Request) {
  try {
    ...
    // 使用 Prisma.TaskWhereInput 類型定義 whereClauses
    ...
    const booleanFields = ["task_apply", "showprice", "completed", "task_public"];
    const numberFields = ["task_price"]; // 數字字段
    const arrayFields = ["School_name"]; // 數組字段
    ...
    // 處理數字字段（task_price）
    if (searchField === "all" || numberFields.includes(searchField)) {
      const numberQuery = parseInt(query, 10);
      if (!isNaN(numberQuery)) {
        whereClauses.OR!.push({ task_price: numberQuery });
      }
