...
export async function GET(req: Request) {
  try {
    ...
    const searchField = searchParams.get("field") || "all";
    const userId = searchParams.get("userId") || ""; // 從查詢參數中獲取 userId

    console.log("userid : ", userId);

    if (!userId) {
      return NextResponse.json({ message: "用戶ID缺失" }, { status: 400 });
    }

    // 根據 userId 獲取用戶的 nickname
    const user = await db.user.findUnique({
      where: { id: userId },
      select: { nickname: true }, // 只獲取 nickname
    });
    ...
    // 明確初始化 whereClauses，確保 AND 是陣列
    let whereClauses: Prisma.JobWhereInput = {
      AND: [
        {
          OR: [
        ...
        { job_user_id: userId }, // 限制 job_user_id 匹配 userId
      ],
    };

    // 安全檢查 OR 是否為空
    // const orConditions = (whereClauses.AND[0] as Prisma.JobWhereInput).OR as Prisma.JobWhereInput[] | undefined;
    ...
    // }
if (
  !Array.isArray(whereClauses.AND) || 
  typeof whereClauses.AND[0] !== 'object' || 
  !('OR' in whereClauses.AND[0])
) {
  whereClauses = {
    AND: [
      { authorname: userNickname },
  ...
  const orConditionList = (whereClauses.AND[0] as { OR: Prisma.JobWhereInput[] }).OR;
  if (!orConditionList || orConditionList.length === 0) {
    whereClauses = {
      AND: [
        { authorname: userNickname },
        { job_user_id: userId },
