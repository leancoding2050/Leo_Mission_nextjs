...
export async function GET(req: Request) {
  try {
    ...
    // 使用 Prisma.UserWhereInput 類型定義 whereClauses
    ...
    // 處理枚舉字段（role）
    if (searchField === "all" || enumFields.includes(searchField)) {
    ...
    if (whereClauses.OR.length === 0) {
      return NextResponse.json([]); // 如果沒有條件，返回空數組
    }
